...
inline int x_DecodeBase64Char(unsigned int nCode)
{
	if (nCode >= 'A' && nCode <= 'Z')		return nCode - 'A';
	if (nCode >= 'a' && nCode <= 'z')		return nCode - 'a' + 26;
	if (nCode >= '0' && nCode <= '9')		return nCode - '0' + 52;
	if (nCode == '+')		return 62;
	if (nCode == '/')		return 63;
	return 64;
}
...
{
    const unsigned char* pbData = data;
    const unsigned char* pbEnd = data + data_len;
    unsigned char* pbOutStart = buf;
    unsigned char* pbOutEnd = buf + buf_len;
    int nFrom = 0;
    ...
    while (pbData < pbEnd)
    {
        ...
        if (ch == '\r' || ch == '\n')
            continue;
        ch = (unsigned char) x_DecodeBase64Char(ch);
...
int x_base64e(unsigned char* buf, int buf_len, unsigned char* data, int data_len)
{
    ...
    for (nFrom=0; nFrom<data_len; nFrom++)
    {
        if (buf >= pbOutEnd)
            break;  
        unsigned char ch = data[nFrom];
        switch (nFrom % 3)
        {       
        case 0: 
            *buf++ = s_Base64Table[ch >> 2];
            chHigh4bits = (ch << 4) & 0x30; 
            break;  
        case 1: 
            *buf++ = s_Base64Table[chHigh4bits | (ch >> 4)]; 
            chHigh4bits = (ch << 2) & 0x3c;
            break;
        default:
            *buf++ = s_Base64Table[chHigh4bits | (ch >> 6)];
            if (buf < pbOutEnd)
            {
                *buf++ = s_Base64Table[ch & 0x3f];
                nLineLen++;
            }
        }
    }
    if (nFrom % 3 != 0 && buf < pbOutEnd)  // 76 = 19 * 4, so the padding wouldn't exceed 76
    {
        *buf++ = s_Base64Table[chHigh4bits];
        int nPad = 4 - (nFrom % 3) - 1;
        if (buf+nPad <= pbOutEnd)
        {
            memset(buf, '=', nPad);
            buf += nPad;
        }
...
{
	assert((mode == MTX_ENCRYPT) || (mode == MTX_DECRYPT));
	
	//keybits legal values are 128, 192 and 256.
...
int aes_encrypt_string(struct _aesParam *ap, uint8_t* src, int src_len, uint8_t *dst, int dst_len)
{
	//
	//   ****************** ******************   ....    |  EXTRA   
	//   |   16 bytes     | |   16 bytes     |   16*n    | 1 bytes 
	//
	//do not accept empty string.  ?
	assert(src_len > 0); 

	//in aes encrypt  16 bytes (4 * 4 uint32_t) is a group. 
	//at first, i calc how  many groups. 
	int groups = (src_len + 15) / 16; 

	//use 1 bytes at dst's tail to explain frags bytes.
	int expect_len = groups * 16 + 1; 
	
	//
	if (dst_len < expect_len)
	{
		//dst_len is insufficient.  cannot encrypt. 
		return -1; 
	}

	int extra = src_len % 16;
	int tail = (extra > 0) ? (16 - extra) : 0;

	int i; 
	for (i = 0; i < groups; i++)
	{
		//note that extra chars will be encrypt. 
		uint32_t t[4];
		int len = 16;
		memset(t, (uint8_t)tail, 16);

		if (i == (groups - 1) && extra != 0)
		{
			len = extra;
		}
		memcpy(t, (uint32_t*)(src + i * 16), len);

		//
		aes_encrypt(ap,  t, (uint32_t*)(dst + i * 16));
	}

	//
	//but the above method will encrypt extra chars at last group.
	//so i'll calc how many extra chars? 
	uint8_t frags = src_len % 16; 	
	dst[expect_len - 1] = (frags > 0) ? (16 - frags) : 0; 
...
int aes_decrypt_string(struct _aesParam *ap, uint8_t* src, int src_len, uint8_t *dst, int dst_len)
{
	...
	//get extra chars. 
	uint8_t extra = src[src_len - 1];
	
	//
	int group = src_len / 16; 
	
	//
	int expect_len = group * 16; 
	...
	{
		//note that extra chars will be encrypt. 
		uint32_t* t = (uint32_t*)(dst + i * 16);
		aes_decrypt(ap, (uint32_t*)(src + i * 16), t);
	}

	//sub extra
	return expect_len - extra; 
}

bool cyt_initialize(struct crypto* p, uint8_t* key, int key_len)
{
	//setup aes encrypt key and decrypt key.
	//
	//at first, setup 256 bits buffer for store key. 
	uint8_t key_buf[CRYPTO_KEY_BYTES];

	//must zero the buffer.
	memset(key_buf, 0, sizeof(key_buf));
		
	//only accept key.
	assert(key_len <= CRYPTO_KEY_BYTES); 

	//copy key to buffer, and left remainning zero.
	int i;
	for (i = 0; i < key_len; i++)
	{
		key_buf[i] = CYT_RAND_DECRYPT(key[i]);
	}

	//setup aes encrypt key.
	if (aes_setup(&p->encrypt_ap, key_buf, CRYPTO_KEY_BITS, MTX_ENCRYPT) < 0) 
	{
		return false; 
	}
	
	//setup aes decrypt key.
	if (aes_setup(&p->decrypt_ap, key_buf, CRYPTO_KEY_BITS, MTX_DECRYPT) < 0) 
	{
		return false; 
	}
...
{
	//aes encode
	uint8_t buf[MTX_BUFFER_SIZE];

	int len = aes_encrypt_string(&p->encrypt_ap, src, src_len, buf, MTX_BUFFER_SIZE); 

	if (len < 0)
		return -1;  //can not encode.

	return x_base64e(dst, dst_len, buf, len);
}
...
int cyt_decrypt(struct crypto* p, uint8_t* src, int src_len, uint8_t* dst, int dst_len)
{
	uint8_t buf[MTX_BUFFER_SIZE];
	int len;
	if ((len = x_base64d(buf, MTX_BUFFER_SIZE, src, src_len)) < 0)
		return -1; 

	return aes_decrypt_string(&p->decrypt_ap, buf, len, dst, dst_len); 
}

struct crypto* init()
{
	uint8_t* key = "\x0c\x0c\x05\x80\x00\x01\x19\xa5\x0c\x0c\x05\x80\x00\x01\x19\xa5";
	int key_len = 16;

	struct crypto* p = (struct crypto*)malloc(sizeof(struct crypto));
	if (p == NULL)
	{
		//init malloc error
		return NULL;
	...
	if (!cyt_initialize(p, key, key_len))
	{
		//init error
		return NULL;
	}
...
JNIEXPORT jstring JNICALL Java_AESUtil_encrypt
  (JNIEnv* env, jclass jc, jstring str, jint src_len){

	 char *src = NULL;
	 src = jstringToChar(env,str);
	// printf("%s\n", src);
	 struct crypto* p = init();
	if (p == NULL)
	{
		//init error
		return NULL;
	...
	if (dst == NULL)
	{
		//encrypt malloc error
		clean(p);
		return NULL;
	}
	memset(dst, '\0', 32);

	int result_len = cyt_encrypt(p, src, src_len, dst, 32);
	if (result_len < 0)
	{
		//encrypt error
		clean(p);
	...
	clean(p);
	dst[result_len] = '\0';
	jstring jstrRe = stoJstring(env, dst);
	return jstrRe;
...
JNIEXPORT jstring JNICALL Java_AESUtil_decrypt
  (JNIEnv* env, jclass jc, jstring str, jint src_len){
	...
	if (dst == NULL)
	{
		//decrypt malloc error
		clean(p);
		return NULL;
	}
	memset(dst, '\0', 32);

	int result_len = cyt_decrypt(p, src, src_len, dst, 32);
	if (result_len < 0)
	{
		//decrypt error
		clean(p);
